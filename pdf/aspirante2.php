...
$registros2=mysqli_query($conexion,"select * from aspirante where cedula_ciudadania='$id'") or
  die("Problemas en el select:".mysqli_error($conexion));

if ($reg2=mysqli_fetch_array($registros2))
{
  $id_aspirante=$reg2['id_e'];
  $nombre=$reg2['nombre'];
...
$registrosy=mysqli_query($conexion,"select * from foto_aspirante where id_aspirante='$cedula'") or
  die("Problemas en el select:".mysqli_error($conexion));

if ($regy=mysqli_fetch_array($registrosy))
{
    $imagen=$regy['imagen'];
}
...
$pic = 'data://text/plain;base64,' . base64_encode($imagen);
$info = getimagesize($pic);
$pdf->Image($pic, 160, 10, 30, 40, 'jpg');


$pdf->Cell(180,6, 'Nombre y Apellido:'.utf8_decode($nombre).'  '.utf8_decode($apellido),1,1,'c',0);
$pdf->Cell(180,6, 'Cedula:'.utf8_decode($cedula),1,1,'c',0);
...
$pdf->Cell(180,6, 'Direccion:'.utf8_decode($direccion),1,1,'c',0);
$pdf->Cell(180,6, 'Telefono:'.utf8_decode($telefono),1,1,'c',0);
...
$registros3=mysqli_query($conexion,"select * from estudios where ASPIRANTE_id_e='$cedula'") or
  die("Problemas en el select:".mysqli_error($conexion));
...
$registros3=mysqli_query($conexion,"select * from experiencia_laboral where aspirante_id_e='$cedula'") or
  die("Problemas en el select:".mysqli_error($conexion));
...
$registros4=mysqli_query($conexion,"select * from referencias_personales where aspirante_id_e='$cedula'") or
  die("Problemas en el select:".mysqli_error($conexion));
...
$pdf->Cell(80,6, 'Telefono:  '.utf8_decode($reg4['telefono_referencia']),1,1,'c',0);
}
